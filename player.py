...
class Player(pygame.sprite.Sprite):
	...
	def check_contact(self):
		...
		semi_colide_rect = [sprite.rect for sprite in self.semi_collision_sprites]

		# collisions
		self.on_surface['floor'] = True if floor_rect.collidelist(collide_rects) >= 0 or floor_rect.collidelist(semi_colide_rect) >= 0 and self.direction.y >= 0 else False
			...
			if sprite.rect.colliderect(self.hitbox_rect):
				if axis == 'horizontal':
					...
					if self.hitbox_rect.top <= sprite.rect.bottom and int(self.old_rect.top) >= int(sprite.old_rect.bottom):
						...
						if hasattr(sprite, 'moving'): # making interaction with platform moving down normal
